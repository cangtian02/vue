...
app.use(favicon('./public/favicon-32x32.png')) // icon
app.use('/dist', serve('./dist', true)) // 静态资源
app.use('/public', serve('./public', true)) // 静态资源
app.use('/manifest.json', serve('./manifest.json', true))
app.use('/service-worker.js', serve('./dist/service-worker.js'))

app.get('*', (req, res) => {
  // 未渲染好返回
  if (!renderer) return res.end('努力编译中...')
  ...
  renderer.renderToStream({title, url: req.url})
    ...
    .on('end', () => console.log(`whole request: ${Date.now() - time}ms`))
    .pipe(res)
})
